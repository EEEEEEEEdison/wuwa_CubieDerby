...
- `21` / `augusta`: 奥古斯塔. Her skill does not activate in round `1`. After that, at action start, if she is the leftmost non-NPC runner in a shared cell, she skips this turn. Her next turn is forced to be last in the round, and that next turn does not check her own skill again.
...
- `23` / `phrolova`: 弗洛洛. Her skill does not activate in round `1`. After that, at action start, if she is the rightmost non-NPC runner in a shared cell, she gains `+3` steps for that turn.
