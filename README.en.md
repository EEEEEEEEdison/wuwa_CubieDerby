...
- `1` / `jinhsi`: 今汐. After another turn changes the final position around her, if 今汐 ends in the same cell and there are valid non-NPC runners to her left, 今汐 makes a `40%` check; on success, she moves to the far left of that cell. This uses the final post-resolution position, so shuffle-cell reordering and NPC turns that carry runners to her left can both satisfy the condition. Turns with no movement normally do not trigger 今汐, except when that no-move turn still triggers a shuffle cell and the reshuffle leaves runners to her left. Luno's later gather effect does not trigger 今汐 by itself.
...
- `21` / `augusta`: 奥古斯塔. At action start, if she is the leftmost non-NPC runner in a shared cell, she skips this turn. Her next turn is forced to be last in the round, and that next turn does not check her own skill again.
- `22` / `luno`: 尤诺. Once per race, and only after her own active turn ends, she checks whether she has already passed cell `17`. If so, the skill only triggers when there is at least one non-NPC runner ranked ahead of her and at least one non-NPC runner ranked behind her. When it triggers, every non-NPC runner is gathered into her current cell, ordered by the pre-teleport ranking from highest to lowest. Otherwise, the skill is kept and checked again after her next active turn.
- `23` / `phrolova`: 弗洛洛. At action start, if she is the rightmost non-NPC runner in a shared cell, she gains `+3` steps for that turn.
