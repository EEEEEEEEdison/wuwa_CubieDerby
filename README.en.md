...
- `1` / `jinhsi`: 今汐. She only checks her skill after another non-NPC runner finishes moving on their own turn, and only when that acting runner ends in the same cell and immediately to her left. On a successful `40%` check, 今汐 moves to the far left of that cell. This still uses the final post-movement position, so a shuffle cell can qualify if the acting runner lands there and ends up immediately to 今汐's left after the reshuffle. NPC turns, no-move turns, shuffle effects from staying in place, and Luno's later gather effect do not trigger 今汐.
