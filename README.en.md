...
- `1` / `jinhsi`: 今汐. After another non-NPC runner has actually moved during their turn, if the final position from that movement resolves into 今汐's cell and places that runner to her left, 今汐 makes a `40%` check; on success, she moves to the far left of that cell. This uses the final post-movement position, so shuffle-cell reordering can satisfy the condition. Turns with no movement normally do not trigger 今汐, except when that no-move turn still triggers a shuffle cell and the reshuffle leaves runners to her left. Luno's later gather effect does not trigger 今汐 by itself.
