            ...
            if player == NPC_ID:
                if npc_active:
                    ...
                    maybe_trigger_player1_skill_after_action(
                        ...
                        actor=NPC_ID,
                        actor_moved=True,
                        allow_shuffle_reorder_trigger=False,
                        track_length=track_length,
                        rng=rng,
                        disabled_skills=config.disabled_skills,
                        skill_state=skill_state,
...
def maybe_trigger_player1_skill_after_action(
    *,
    ...
    if actor == JINHSI_ID or JINHSI_ID not in progress or not skill_enabled_from_set(disabled_skills, JINHSI_ID):
        return
    ...
    if trace:
        log_timing(trace, "行动结束", f"{format_runner(JINHSI_ID)}检查行动角色{format_runner(actor)}是否位于自己左侧")
    cell = grid.get(pos)
    ...
    if actor == NPC_ID:
        left_runners = [runner for runner in cell[:one_idx] if runner not in (NPC_ID, JINHSI_ID)]
        if not left_runners:
            if trace:
                log_block(
                    trace,
    ...
    if actor not in cell or cell.index(actor) >= one_idx:
        if trace:
            log_block(
                ...
                f"行动角色：{format_runner(actor)}",
                f"位置：{format_position(pos)}",
                f"原因：行动角色不在{format_runner(JINHSI_ID)}左侧",
                f"格内顺序：{format_cell(cell)}",
            )
        return
