...
- `20` / `aemeath`: 爱弥斯. Once per race, only during 爱弥斯's own action, if her moving group moves forward through cell `17` and another non-NPC runner is ahead of cell `17`, only 爱弥斯 teleports to the nearest such runner's cell and enters from the left. If another runner carries her through cell `17`, her skill does not trigger. If 爱弥斯 is the active runner while carrying others, the carried runners stop on cell `17`, and 爱弥斯 continues any remaining movement after teleporting. If no valid target exists, the skill is not consumed.
