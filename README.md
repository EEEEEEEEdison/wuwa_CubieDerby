...
- `21` / `augusta`：奥古斯塔。第 1 回合不发动技能。之后在行动开始时，若自己位于同格最左侧，且同格存在其他非 NPC 角色，则本回合不行动；同时下一回合固定最后行动，并且该回合不再判定自身技能。
...
- `23` / `phrolova`：弗洛洛。第 1 回合不发动技能。之后在行动开始时，若自己位于同格最右侧，且同格存在其他非 NPC 角色，则本回合额外前进 `3` 格。
